...
Banco::Banco(string nome, string cnpj, string nomeBanco): PessoaJuridica(nome, cnpj), nome(nomeBanco){
	ifstream fin("01dados.dat", ios::in);
	if(!fin) throw runtime_error("/!\\ Erro!, não consegui acessar o arquivo de dados!");
	...
	for(int i = 0 ; i < qtdPessoas ; i++){
		...
		if(linhaPessoa.ident == 1) cadastrarPessoa(linhaPessoa.nome, linhaPessoa.dado, "fisica");
		else cadastrarPessoa(linhaPessoa.nome, linhaPessoa.dado, "juridica");
	}
	...
	for(int i = 0 ; i < qtdContas ; i++){
		fin >> linhaConta.tipo >> linhaConta.propietario >> linhaConta.nmrConta >> linhaConta.saldo >> linhaConta.limite >> linhaConta.dia >> linhaConta.qtdTransacoes;
		
		if(linhaConta.tipo == "ContaCorrente."){
			cadastrarConta(getPessoa(linhaConta.propietario), linhaConta.nmrConta, linhaConta.saldo);
			for(int i = 0 ; i < linhaConta.qtdTransacoes ; i++){
				fin >> linhaTransacoes.tipo >> linhaTransacoes.data >> linhaTransacoes.descricao >> linhaTransacoes.valor;
			...
			cadastrarConta(getPessoa(linhaConta.propietario), linhaConta.nmrConta, linhaConta.saldo, linhaConta.limite);
			for(int i = 0 ; i < linhaConta.qtdTransacoes ; i++){
				fin >> linhaTransacoes.tipo >> linhaTransacoes.data >> linhaTransacoes.descricao >> linhaTransacoes.valor;
			...
			cadastrarConta(getPessoa(linhaConta.propietario), linhaConta.nmrConta, linhaConta.saldo, linhaConta.dia);
			for(int i = 0 ; i < linhaConta.qtdTransacoes ; i++){
				fin >> linhaTransacoes.tipo >> linhaTransacoes.data >> linhaTransacoes.descricao >> linhaTransacoes.valor;
...
Banco::~Banco(){
	...
	for(int i = 0 ; i < this->contas.size() ; i++){
		if(this->contas[i]->getTipo() == "ContaCorrente."){
			...
			ContaCorrenteLimitada *c = dynamic_cast<ContaCorrenteLimitada*>(this->contas[i]);
			fout << *c;
		}
		else{
...
Banco & Banco::cadastrarConta(Pessoa* pessoa, int nmr, double saldo, double limite){
	for(auto &n : this->contas){
		if(n->getNmr() == nmr)
			throw runtime_error("	/!\\ Esse número já |EXISTE NO SISTEMA|!");
	}
...
Banco & Banco::removerConta(int num){
	...
	throw runtime_error("/!\\		|CONTA| não encontrada!, insira informações |VÁLIDAS| na próxima vez.");
}
...
Banco & Banco::atualizarConta(Pessoa* novaPessoa, double novoSaldo, int numConta){
	for(int i = 0 ; i < this->contas.size() ; i++)
	...
	throw("/!\\		|CONTA| não encontrada!, insira informações |VÁLIDAS| na próxima vez.");
	return *this;
}
...
Banco & Banco::listarContasCorrentista(string correntista){
	...
	if(aux)	
		throw runtime_error("/!\\		|CORRENTISTA| não encontrado!, insira informações |VÁLIDAS| na próxima vez.");
	return *this;
}
...
Banco & Banco::listarPessoasCadastradas(){
	cout << "\nPessoas |CADASTRADAS NO SISTEMA|:" << endl << "------------------------------" << endl;
	for(auto &n : this->pessoas)
		n->print();
	return *this;
}
...
Conta* Banco::getConta(int num){
	...
	throw runtime_error("/!\\		|CONTA| não encontrada!, insira informações |VÀLIDAS| na próxima vez.");
}
...
Pessoa* Banco::getPessoa(string nome){
	...
	throw runtime_error("/!\\		|PESSOA| não encontrada!, insira informações |VÀLIDAS| na próxima vez.");
}
